// =============================================
// SMART LOCK - ESP32 + BLYNK (FINAL v9.0 FIXED)
// RGB LED active LOW (common anode)
//   GPIO0 = ĐỎ, GPIO2 = XANH
// Mật khẩu mặc định: 5678
// Keypad: A=reset, B=đổi pass, C=enroll vân tay, D=toggle PIR alert, *=clear, #=enter
// Blynk Events: gas_alert, wrong_pass_alert, fingerprint_alert, pir_alert
// V20 = Button mở/đóng cửa
// V30 = Door status String ("Opened"/"Closed")
// V40 = Alert enable (1=bật, 0=tắt) — chỉ gas
// V50 = Nhiệt độ, V51 = Thời tiết
// V70 = Auto-lock delay (Slider, 5–300 giây)
// V71 = Auto-lock enable (Button Switch)
// V100 = Finger ID, V101 = Enroll, V102 = Delete, V103 = Status
// V104 = Text Input tên người dùng
// V41 = PIR Alert Mode (0=OFF, 1=ON)
// V3 = LDR Value, V4 = PIR Status
// LED Matrix: "Normal" tĩnh / "ALERT!!" tĩnh
// LCD: TIME(8s) → DATE(4s) → WEATHER(4s)
// PIR + tối → bật đèn xanh 10s tự tắt (alert OFF)
//          → báo động 7s (alert ON, cần pass để tắt qua phím D hoặc V41)
// Auto-lock mặc định 7s
// Keypad lockout: sai 5 lần → khóa 30s
//
// ✨ NEW v9.1: Telemetry gửi trực tiếp về backend
// =============================================
...
// Doi IP nay thanh IPv4 LAN cua may dang chay backend.
// Vi du: ipconfig -> IPv4 Address: 192.168.1.100
...
// - Double temperature
// - String weatherDesc
// Backend saves to sensor_data and broadcasts WebSocket telemetry.
// =========================================================
void sendTelemetryToBackend(int gasVal, int ldrVal, int pirVal) {
  ...
  doc["temperature"] = (double)weatherTemp;
...
void sendSensorData() {
  ...
  // Blynk telemetry, giu nguyen luong hien co.
  ...
  // Backend telemetry, dung 6 fields khop DeviceReportDTO.
...
BLYNK_WRITE(V70) {
  ...
  char buf[24];
  snprintf(buf, sizeof(buf), "  After %3ds    ", s);
...
bool enrollFingerprint(int id) {
  ...
  if (dupID >= 0) {
    ...
    char buf[24];
    snprintf(buf, sizeof(buf), " Already ID:%d", dupID);
    lcd.setCursor(0,1); lcd.print(buf);
  ...
  if (!fingerPresent) {
    ...
    if (p == FINGERPRINT_OK) {
      ...
      if (fingerFailCount >= MAX_FINGER_FAIL) {
        ...
        char failBuf[24];
        snprintf(failBuf, sizeof(failBuf), "Fail: %d/%d", fingerFailCount, MAX_FINGER_FAIL);
...
void handleNormalDisplay() {
  ...
  switch (displayMode) {
    case 0: {
      ...
      char buf[17]; snprintf(buf, sizeof(buf), "   %02d:%02d:%02d   ", hh, mm, ss);
    ...
    case 1: {
      ...
      char buf[17]; snprintf(buf, sizeof(buf), "  %02d/%02d/%04d  ", ti->tm_mday, ti->tm_mon+1, ti->tm_year+1900);
    ...
    case 2: {
      ...
      snprintf(line0, sizeof(line0), " Temp: %.1f C   ", weatherTemp);
...
void setup() {
  ...
  lcd.setCursor(0,1); lcd.print(" Press # to open");
  matrixLastText = ""; matrixSetNormal();
  delay(1000); Serial.println("--- SYSTEM READY (v9.1 with Telemetry) ---");
}
...
void loop() {
  ...
  if (keypadLocked) {
    if (millis() - keypadLockedAt >= KEYPAD_LOCK_DURATION) {
      ...
      if (key) {
        ...
        lcd.setCursor(0,0); lcd.print(" Keypad LOCKED! ");
        char buf[24];
        snprintf(buf, sizeof(buf), "  Wait: %2lus    ", remaining);
        lcd.setCursor(0,1); lcd.print(buf);
  ...
  if (!enrollingNow && !keypadLocked && key && millis() - lastKeyTime > 300) {
    ...
    // ─── D: TOGGLE PIR ALERT ─────────────────────────────
      ...
      if (!pirAlertEnabled) {
        ...
        Blynk.virtualWrite(VPIN_PIR_ALERT_MODE, 1);
        lcd.clear();
    ...
    // ─── #: ENTER ────────────────────────────────────────
      ...
      if (sysState == STATE_KEY_D_AUTH) {
        if (inputPass == password) {
          ...
          Blynk.virtualWrite(VPIN_PIR_ALERT_MODE, 0);
          lcd.clear();
          lcd.setCursor(0,0); lcd.print(" PIR  Alert:    ");
        ...
        if (enrollIDStr.length() == 0 || enrollID < 1 || enrollID > 127) {
          ...
          lcd.setCursor(0,0); lcd.print("  Enrolling...  ");
          char buf[24];
          snprintf(buf, sizeof(buf), "   ID: %-3d      ", enrollID);
          lcd.setCursor(0,1); lcd.print(buf);
          enrollIDStr = "";
  ...
  if (millis() - lastConsoleTime > 1000) {
    ...
    Serial.printf("[GAS]%d [LDR]%d [PIR]%d [LIGHT]%s [DOOR]%s [AUTO-LOCK]%s(%lus) [PIR-MODE]%s [PIR-ACTIVE]%s [KP-LOCK]%s\n",
